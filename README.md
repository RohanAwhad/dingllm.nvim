...
```
	{
		...
		config = function()
			...
			local function together_generate()
			...
			local function together_replace()
				dingllm.invoke_llm_and_stream_into_editor({
					url = "https://api.together.xyz/v1/chat/completions",
					model = "meta-llama/Meta-Llama-3.1-70b-Instruct-Turbo",
					api_key_name = "TOGETHER_API_KEY",
			...
			vim.keymap.set({ "n", "v" }, "<leader>t", together_generate, { desc = "llm together generate" })
			vim.keymap.set({ "n", "v" }, "<leader>tr", together_replace, { desc = "llm together replace" })
			vim.keymap.set({ "n", "v" }, "<leader>th", together_help, { desc = "llm together help" })

			vim.keymap.set({ "n", "v" }, "<leader>k", groq_generate, { desc = "llm groq generate" })
			vim.keymap.set({ "n", "v" }, "<leader>kr", groq_replace, { desc = "llm groq replace" })
			vim.keymap.set({ "n", "v" }, "<leader>kh", groq_help, { desc = "llm groq_help" })
